...
serve(async (req) => {
  ...
  try {
    ...
    for (const [i, e] of newEmails.entries()) {
      ...
      const aiRelevance = isGroqProvider()
        ? await groqIsRelevant(e.subject, e.body)
        : await aiIsRelevant(e.subject, e.body);
      if (!aiRelevance.relevant) {
        irrelevantIds.push(e.id);
        continue;
      }

      let analysis = isGroqProvider()
        ? await categorizeWithGroq(e.subject, e.body)
        : await categorizeWithOllama(e.subject, e.body);
      if (!analysis) {
